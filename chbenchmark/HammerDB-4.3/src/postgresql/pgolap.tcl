...
proc CreateTables { lda greenplum gpcompress } {
    ...
    if { $greenplum } {
		...
		if 1 {
        set sql(1) "CREATE TABLE ORDERS (O_ORDERDATE TIMESTAMP, O_ORDERKEY NUMERIC NOT NULL, O_CUSTKEY NUMERIC NOT NULL, O_ORDERPRIORITY CHAR(15), O_SHIPPRIORITY NUMERIC, O_CLERK CHAR(15), O_ORDERSTATUS CHAR(1), O_TOTALPRICE NUMERIC, O_COMMENT VARCHAR(79)) USING HAP"
        set sql(2) "CREATE TABLE PARTSUPP (PS_PARTKEY NUMERIC NOT NULL, PS_SUPPKEY NUMERIC NOT NULL, PS_SUPPLYCOST NUMERIC NOT NULL, PS_AVAILQTY NUMERIC, PS_COMMENT VARCHAR(199)) USING HAP"
        set sql(3) "CREATE TABLE CUSTOMER(C_CUSTKEY NUMERIC NOT NULL, C_MKTSEGMENT CHAR(10), C_NATIONKEY NUMERIC, C_NAME VARCHAR(25), C_ADDRESS VARCHAR(40), C_PHONE CHAR(15), C_ACCTBAL NUMERIC, C_COMMENT VARCHAR(118)) USING HAP"
        set sql(4) "CREATE TABLE PART(P_PARTKEY NUMERIC NOT NULL, P_TYPE VARCHAR(25), P_SIZE NUMERIC, P_BRAND CHAR(10), P_NAME VARCHAR(55), P_CONTAINER CHAR(10), P_MFGR CHAR(25), P_RETAILPRICE NUMERIC, P_COMMENT VARCHAR(23)) USING HAP"
        set sql(5) "CREATE TABLE SUPPLIER(S_SUPPKEY NUMERIC NOT NULL, S_NATIONKEY NUMERIC, S_COMMENT VARCHAR(102), S_NAME CHAR(25), S_ADDRESS VARCHAR(40), S_PHONE CHAR(15), S_ACCTBAL NUMERIC) USING HAP"
        set sql(6) "CREATE TABLE NATION(N_NATIONKEY NUMERIC NOT NULL, N_NAME CHAR(25), N_REGIONKEY NUMERIC, N_COMMENT VARCHAR(152)) USING HAP" 
        set sql(7) "CREATE TABLE REGION(R_REGIONKEY NUMERIC, R_NAME CHAR(25), R_COMMENT VARCHAR(152)) USING HAP"
        set sql(8) "CREATE TABLE LINEITEM(L_SHIPDATE TIMESTAMP, L_ORDERKEY NUMERIC NOT NULL, L_DISCOUNT NUMERIC NOT NULL, L_EXTENDEDPRICE NUMERIC NOT NULL, L_SUPPKEY NUMERIC NOT NULL, L_QUANTITY NUMERIC NOT NULL, L_RETURNFLAG CHAR(1), L_PARTKEY NUMERIC NOT NULL, L_LINESTATUS CHAR(1), L_TAX NUMERIC NOT NULL, L_COMMITDATE TIMESTAMP, L_RECEIPTDATE TIMESTAMP, L_SHIPMODE CHAR(10), L_LINENUMBER NUMERIC NOT NULL, L_SHIPINSTRUCT CHAR(25), L_COMMENT VARCHAR(44)) USING HAP"
		} else {
...
proc mk_partsupp { lda start_rows end_rows scale_factor greenplum } {
	if { $greenplum } { set rowcount 1 } else { set rowcount 1000 }
    for { set i $start_rows } { $i <= $end_rows } { incr i } {
		set partkey $i
		#Part Supp Loop
	...
	puts "PARTSUPP Done Rows $start_rows..$end_rows"
    return
}
...
proc mk_part { lda start_rows end_rows scale_factor greenplum } {
    ...
    puts "PART Done Rows $start_rows..$end_rows"
    return
}
...
proc do_tpch { host port sslmode scale_fact superuser superuser_password defaultdb db tspace user password greenplum gpcompress num_vu } {
        ...
        switch $myposition {
            1 { 
                ...
                if { $threaded eq "MULTI-THREADED" } {
					...
					# Flags to control wait loops.
                    tsv::set application load "WAIT"
					tsv::set application partload "WAIT"
					tsv::set application partsuppload  "WAIT"
					tsv::set application orderload "WAIT"
                }
            }
            default { 
                puts "Worker Thread"
    ...
    if { $threaded eq "SINGLE-THREADED" ||  $threaded eq "MULTI-THREADED" && $myposition eq 1 } {
        ...
        if { $threaded eq "MULTI-THREADED" } {
			...
			# Encode REGION table's attribute.
			set result [ pg_exec $lda "SELECT hap_encode('public.region.r_name')"]
			set result [ pg_exec $lda "commit" ]
			pg_result $result -clear
			puts "Encoding REGION COMPLETE"
			...
			# Encode NATION table's attribute.
			set result [ pg_exec $lda "SELECT hap_encode('public.nation.n_name')"]
			set result [ pg_exec $lda "commit" ]
			pg_result $result -clear
			puts "Encoding NATION COMPLETE"

			# Release worker threads from the wait loop.
            tsv::set application load "READY"

            puts "Monitoring Workers..."
            after 10000
            while 1 {
				set custdncnt 0; set partdncnt 0; set partsuppdncnt 0; set dncnt 0;
                for {set th 2} {$th <= $totalvirtualusers } {incr th} {
                    switch [tsv::lindex common thrdlst $th] {
                        custdone { incr custdncnt }
                        partdone { incr partdncnt }
						partsuppdone { incr partsuppdncnt }
                        done { incr dncnt }
                    }
                }
				if { $custdncnt eq [expr $totalvirtualusers - 1] } {
					# Encode CUSTOMER table's attribute.
					puts "Start encoding CUSTOMER table"
					set result [ pg_exec $lda "SELECT hap_encode('public.customer.c_mktsegment')"]
					set result [ pg_exec $lda "commit" ]
					pg_result $result -clear

					# Release the workers from wait loop.
					puts "PART/PARTSUPP loading is enabled"
					tsv::set application partload "READY"
				}
				if { $partdncnt eq [expr $totalvirtualusers - 1] } {
					# Encode PART table's attribute.
					puts "Start encoding PART table"
					set result [ pg_exec $lda "SELECT hap_encode('public.part.p_size')"]
					set result [ pg_exec $lda "commit" ]
					pg_result $result -clear
					puts "Encoding PART_SIZE COMPLETE"
					set result [ pg_exec $lda "SELECT hap_encode('public.part.p_type')"]
					set result [ pg_exec $lda "commit" ]
					pg_result $result -clear
					puts "Encoding PART_TYPE COMPLETE"
					set result [ pg_exec $lda "SELECT hap_encode('public.part.p_brand')"]
					set result [ pg_exec $lda "commit" ]
					pg_result $result -clear
					puts "Encoding PART_BRAND COMPLETE"
					set result [ pg_exec $lda "SELECT hap_encode('public.part.p_container')"]
					set result [ pg_exec $lda "commit" ]
					pg_result $result -clear
					puts "Encoding PART_CONTAINER COMPLETE"
					puts "Encoding PART COMPLETE"

					# Release the workers from wait loop.
					puts "PARTSUPP loading is enabled"
					tsv::set application partsuppload "READY"
				}
				if { $partsuppdncnt eq [expr $totalvirtualusers - 1] } {
					# Release the workers from wait loop.
					# Note that CUSTOMER table is already build before PART.
					puts "ORDER/LINEITEM loading is enabled"
					tsv::set application orderload "READY"
				}
    ...
    if { $threaded eq "SINGLE-THREADED" ||  $threaded eq "MULTI-THREADED" && $myposition != 1 } {
		...
		if { $threaded eq "MULTI-THREADED" } {
            tsv::lreplace common thrdlst $myposition $myposition custdone
        } else {
			# Encode CUSTOMER table's attribute.
			puts "Start encoding CUSTOMER table"
			set result [ pg_exec $lda "SELECT hap_encode('public.customer.c_mktsegment')"]
		...
		# Wait until supplier and customer tables are completed.
		if { $threaded eq "MULTI-THREADED" } {
			while 1 {
				if { [ tsv::exists application partload ] } {
					if {  [ tsv::get application partload ] eq "READY" } { break }
					if {  [ tsv::get application abort ]  } { return }
				}
				after 5000
			}
			tsv::lreplace common thrdlst $myposition $myposition loading
		}
        puts "Loading PART..."
        mk_part $lda [ lindex $part_chunk 0 ] [ lindex $part_chunk 1 ] $scale_fact $greenplum
		if { $threaded eq "MULTI-THREADED" } {
            tsv::lreplace common thrdlst $myposition $myposition partdone
        } else {
			# Encode PART table's attribute.
			puts "Endoing PART start"
			set result [ pg_exec $lda "SELECT hap_encode('public.part.p_size')"]
			set result [ pg_exec $lda "commit" ]
			pg_result $result -clear
			puts "Encoding PART_SIZE COMPLETE"
		...
		# Wait until part table is completed.
		if { $threaded eq "MULTI-THREADED" } {
			while 1 {
				if { [ tsv::exists application partsuppload ] } {
					if {  [ tsv::get application partsuppload ] eq "READY" } { break }
					if {  [ tsv::get application abort ]  } { return }
				}
				after 5000
			}
		...
		puts "Loading PARTSUPP..."
        mk_partsupp $lda [ lindex $part_chunk 0 ] [ lindex $part_chunk 1 ] $scale_fact $greenplum
		if { $threaded eq "MULTI-THREADED" } {
            tsv::lreplace common thrdlst $myposition $myposition partsuppdone
        }

		# Wait until part table is completed.
		if { $threaded eq "MULTI-THREADED" } {
			while 1 {
				if { [ tsv::exists application orderload ] } {
					if {  [ tsv::get application orderload ] eq "READY" } { break }
					if {  [ tsv::get application abort ]  } { return }
				}
				after 5000
			}
