...
Status TransactionDB::Open(const Options& options,
  ...
  // HEMANT: hack, set to ColumnMerger if nothing set yet
  if(!cf_options.merge_operator) {
    cf_options.merge_operator.reset(new ColumnMerger(cf_options.delim[0], cf_options.column_num));
  }
