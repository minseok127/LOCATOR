...
#ifdef LOCATOR
				/*
				 * XXX ignore not null constraint for columnar layout.
				 *
				 * If we have to check null constraint, we need to access all
				 * the not null constrinat attribute's pages.
				 */
				if (slot->tts_locator_executor)
					continue;
#endif /* LOCATOR */
