...
#ifdef LOCATOR
Datum
print_external_catalog(PG_FUNCTION_ARGS)
{
...
/*
 * built-in function for setting columnar layout
 */
Datum
locator_set_columnar_layout(PG_FUNCTION_ARGS)
{
	Oid relationOid;
	char raw_string[256];
	VarChar *src;
	...
	if (PG_NARGS() != 2)
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("Input the valid argument (relation oid, 'frozen line | update count | group count | group1 len | attr | ... | attr | group2 len | ...')")));
	}

	/* Get the arguments (oid, '3|1|4|2|0|1|...') */
	relationOid = PG_GETARG_OID(0);
	src = PG_GETARG_VARCHAR_PP(1);
	len = VARSIZE_ANY_EXHDR(src);
	instr = VARDATA_ANY(src);

	/* Make a string */
	strncpy(raw_string, instr, len);
	raw_string[len] = '\0';

	LocatorSetColumnarLayout(relationOid, raw_string);
