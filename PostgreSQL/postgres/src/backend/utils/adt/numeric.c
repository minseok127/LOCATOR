...
#ifdef DIVA
/*
 * numeric_maximum_heap_size() -
 *
 *	Maximum size of a numeric in heap page with given typmod, or -1 if
 *	unlimited/unknown.
 */
int32
numeric_maximum_heap_size(int32 typmod)
{
	int			precision;
	int			scale;
	int			weight;
	...
	weight = (precision - scale - 1) / DEC_DIGITS;

	/* this ndigits is same as numeric_digits in numeric_maximum_size() */
	numeric_digits = weight + ((scale - 1) / DEC_DIGITS) + 2;
	ndigit_sz = numeric_digits * sizeof(NumericDigit);

	/* shorten the length if possible */
	if (NUMERIC_CAN_BE_SHORT(scale, weight))
		nmax_sz =  NUMERIC_HDRSZ_SHORT + ndigit_sz;
	else
		nmax_sz = NUMERIC_HDRSZ + ndigit_sz;
	
	/* numeric is varlena, so try to shorten one more time */
	if ((nmax_sz - VARHDRSZ + VARHDRSZ_SHORT) <= VARATT_SHORT_MAX)
		nmax_sz = (nmax_sz - VARHDRSZ + VARHDRSZ_SHORT);
