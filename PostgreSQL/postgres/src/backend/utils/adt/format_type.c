...
#ifdef DIVA
/*
 * type_maximum_heap_size --- determine maximum width of a variable-width column
 * in heap page.
 *
 * If the max width is indeterminate, return -1.  In particular, we return
 * -1 for any type not known to this routine.  We assume the caller has
 ...
 */
int32
type_maximum_heap_size(Oid type_oid, int32 typemod)
{
	if (typemod < 0)
		return -1;
	...
	switch (type_oid)
	{
		...
		case VARCHAROID:
			/* typemod includes varlena header */
			if ((typemod - VARHDRSZ + VARHDRSZ_SHORT) <= VARATT_SHORT_MAX)
				return (typemod - VARHDRSZ + VARHDRSZ_SHORT);
			else
				return typemod;
		case NUMERICOID:
			return numeric_maximum_heap_size(typemod);
