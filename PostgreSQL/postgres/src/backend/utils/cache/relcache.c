...
RelationBuildTupleDesc(Relation relation)
{
	...
	bool		siro;
	uint16		maxlen = 0;
	uint16		alignedmaxlen;
	uint16		alignedmaxdatalen = 0;
	bool		has_null = false;
#ifdef LOCATOR
	uint16		hattrbytesize;
#endif /* LOCATOR */
#endif /* DIVA */
	...
	if (siro)
	{
		...
		maxlen = alignedmaxdatalen = MAXALIGN(maxlen);

#ifdef LOCATOR
		relation->rd_locator_attrs_maxlen =
			(uint16*)MemoryContextAllocZero(CacheMemoryContext,
											need * sizeof(uint16));
#endif /* LOCATOR */
	}
#endif /* DIVA */
...
#ifdef LOCATOR
			{
				if (!strcmp(attp->attname.data, "_hap_hidden_attribute"))
				{
					/* Get the length of hidden attribute */
					hattrbytesize =
						HapRelidGetHiddenAttrBytesize(RelationGetRelid(relation));

					/*
					 * The type of hidden attribute is byte array, so calculate
					 * the size of varlena header.
					 */
					if (likely(hattrbytesize + VARHDRSZ_SHORT <= VARATT_SHORT_MAX))
						tmplen = hattrbytesize + VARHDRSZ_SHORT;
					else
						tmplen = hattrbytesize + VARHDRSZ;
				}
				else
...
#ifdef LOCATOR
			/* Store the maxlen of this attribute */
			relation->rd_locator_attrs_maxlen[attnum - 1] = tmplen;
#endif /* LOCATOR */

			maxlen += tmplen;
			if (attp->attlen >= 0 && attp->atttypid == INT4OID)
				maxlen = att_align_nominal(maxlen, attp->attalign);
		}
#endif /* DIVA */
...
#ifdef DIVA
		if (!attp->attnotnull && 
			strcmp(attp->attname.data, "_hap_hidden_attribute") != 0)
			has_null = true;
#endif
	...
	if (siro)
	{
		...
		if (!has_null)
		{
			maxlen -= MAXALIGN(offsetof(HeapTupleHeaderData, t_bits) + BITMAPLEN(need));
			maxlen += MAXALIGN(offsetof(HeapTupleHeaderData, t_bits));
		}

		alignedmaxlen = MAXALIGN(maxlen);
		alignedmaxdatalen =
			MAXALIGN(RelationGetToastTupleTarget(relation, TOAST_TUPLE_TARGET) -
					 alignedmaxdatalen);

		maxlen = alignedmaxlen < alignedmaxdatalen ?
...
retry:
	...
	/*
	 * Set LOCATOR's transformation related members.
	 */
	relation->rd_locator_partitioning = relp->rellocatorpartitioning;
	relation->rd_locator_part_count = 0;
	relation->rd_locator_level_count = 0;
	relation->rd_locator_spread_factor = 0;
	relation->rd_locator_attrs_maxlen = NULL;
#endif /* LOCATOR */
	...
	/* Build LOCATOR transformation metadata */
	if (relation->rd_locator_partitioning)
	{
		LocatorGetTransformationInfo(relid,
									 &relation->rd_locator_part_count,
									 &relation->rd_locator_level_count,
									 &relation->rd_locator_spread_factor);
	}
#endif /* LOCATOR */
...
#ifdef LOCATOR
	else if (!strncmp(RelationGetRelationName(relation), "_hap_hidden_partition_map_", 26))
		is_siro = false;
#endif /* LOCATOR */
 ...
 * TupleDescGetMaxLen
 * 
 * In certain cases, the TupleDescriptor(relation->rd_att) does not have maxlen
 * stored in it. (discovering this out is a TODO)
 * This function calculates maxlen in those cases.
 * 
 * @param relation 
 */
void
TupleDescGetMaxLen(Relation relation)
{
	HeapTuple	pg_attribute_tuple;
	Relation	pg_attribute_desc;
	SysScanDesc pg_attribute_scan;
 ...
 * IsLocator
 *
 * The return value is true if Locator partitioning is being used in the 
 * relation.
 */
inline bool 
IsLocator(Relation relation)
{
	if (relation == NULL)
		return false;

	return relation->rd_locator_partitioning;
}
#endif /* LOCATOR */
