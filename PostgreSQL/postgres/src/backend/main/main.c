...
static void hooked_segfault_handler(int sig);
static void hooked_abort_handler(int sig);
static void (*default_SIGSEGV)(int);
static void (*default_SIGABRT)(int);

static void
hooked_segfault_handler(int sig)
{
	fprintf(stderr, "segmentation fault, PID: %d, sleep %d secs....\n", getpid(), SLEEP_SECS);
	sleep(SLEEP_SECS);
	default_SIGSEGV(sig);
}

static void
hooked_abort_handler(int sig)
{
	fprintf(stderr, "abort, PID: %d, sleep %d secs....\n", getpid(), SLEEP_SECS);
	sleep(SLEEP_SECS);
	default_SIGABRT(sig);
}
#endif /* HOOK_SIGNAL */
...
#ifdef HOOK_SIGNAL
	default_SIGSEGV = signal(SIGSEGV, hooked_segfault_handler);
	default_SIGABRT = signal(SIGABRT, hooked_abort_handler);
#endif /* HOOK_SIGNAL */
