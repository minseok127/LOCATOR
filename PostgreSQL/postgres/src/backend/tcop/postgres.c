...
#ifdef IO_AMOUNT
uint32		read_nblocks;
uint32		reduced_by_HP;
uint32		reduced_by_VP;
extern bool	isOLAP;
extern char query_no[4];
#endif /* IO_AMOUNT */
...
#ifdef IO_AMOUNT
	if (isOLAP && isdigit(query_no[0]) && isdigit(query_no[1]) && read_nblocks != 0)
	{
		fprintf(stderr, "[IO_AMOUNT] %s read amount, nblocks: %u, reduced by horizontal reduction: %u, reduced by vertical reduction: %u\n",
						query_no, read_nblocks, reduced_by_HP, reduced_by_VP);
	}
#endif /* IO_AMOUNT */
}
