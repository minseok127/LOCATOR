...
/*
 * __HAP_LOCATOR_DefineInternalRel
 *		Create table that is defined by HAP/LOCATOR internally.
 *
 * Tables needed by the internal operation of HAP/LOCATOR are defined, not by
 * user requests, such as partition maps or RID tables.
 */
static Oid
__HAP_LOCATOR_DefineInternalRel(Relation parentrel,
...
HapDefinePartMap(HapPartCreateStmt *stmt, Relation parentrel)
{
	...
	stmt->partmap_id = __HAP_LOCATOR_DefineInternalRel(parentrel,
													   relname, tableElts);	
					  ...
					  LOCKMODE, lockmode)
{
	...
	DefElem *locator_spread_factor = NULL;
	...
	foreach(lc, defList)
	{
		...
		else if (strncmp(def->defname, "locator_spread_factor", 21) == 0)
			locator_spread_factor = def;
	}
	...
	/* Connect it to the locator module */
	LocatorRegisterRel(stmt.target_relid,
					   stmt.part_count, locator_spread_factor);
}
#endif /* LOCATOR */
