		  ...
		  transition_capture->tcs_update_new_table)))
	{
		...
		if (fdw_trigtuple == NULL && ItemPointerIsValid(tupleid))
			GetTupleForTrigger(estate,
							   ...
							   tupleid,
#ifdef LOCATOR
							   newslot->tts_locator_route_synopsis,
#endif
							   LockTupleExclusive,
							   oldslot,
							   NULL,
...
#ifdef LOCATOR_DEBUG
		if (newslot->tts_locator_route_synopsis != NULL)
		{
			Assert(ItemPointerIsValid(&(oldslot->tts_tid)));
			Assert(ItemPointerIsValid(&(newslot->tts_tid)));
		}
#endif /* LOCATOR_DEBUG */
...
#ifdef LOCATOR
		if (IsLocator(tupsrc->ri_RelationDesc) &&
				oldslot->tts_tuple_position->needDecrRefCount)
		{
			LocatorExternalCatalog *exCatalog = 
				LocatorGetExternalCatalog(tupsrc->ri_RelationDesc->rd_node.relNode);
			LocatorPartLevel partLvl = oldslot->tts_tuple_position->partitionLevel;
			LocatorPartNumber partNum = oldslot->tts_tuple_position->partitionNumber;
	...
	else
	{
		...
		if (IsSiro(relation))
		{
			...
			if (isLocator)
			{
				...
				oldslot->tts_locator_route_synopsis = route_synopsis;
			}
#endif
