...
#ifdef LOCATOR
#define BACKEND_TYPE_LOCATOR	BACKEND_TYPE_AUTOVAC
#endif /* LOCATOR */
...
#endif
#ifdef LOCATOR
			LocatorPartitionLauncherPID = 0,
			LocatorBgWriterPID = 0,
#endif
			WalReceiverPID = 0,
			AutoVacPID = 0,
			PgArchPID = 0,
...
#ifdef LOCATOR
static void StartLocatorPartitioningWorker(void);
#endif /* LOCATOR */
...
#endif
#ifdef LOCATOR
#define StartLocatorBackgroundWriter() StartChildProcess(LocatorBgWriterProcess)
#endif
...
#ifdef LOCATOR
	if (LocatorBgWriterPID == 0)
		LocatorBgWriterPID = StartLocatorBackgroundWriter();
#endif
...
#ifdef LOCATOR
		if (LocatorPartitionLauncherPID == 0 && pmState == PM_RUN)
			LocatorPartitionLauncherPID = StartLocatorPartitionLauncher();
#endif
...
#endif
#ifdef LOCATOR
		if (LocatorPartitionLauncherPID != 0)
			signal_child(LocatorPartitionLauncherPID, SIGHUP);
		if (LocatorBgWriterPID != 0)
			signal_child(LocatorBgWriterPID, SIGHUP);
#endif
		if (WalReceiverPID != 0)
...
reaper(SIGNAL_ARGS)
{
	...
	while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0)
	{
...
#ifdef LOCATOR
		/*
		 * Was it the locator bgwriter?  Normal exit can be ignored; 
		 ...
		 */
		if (pid == LocatorBgWriterPID)
		{
			LocatorBgWriterPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("Locator background writer process"));
			continue;
		}
#endif
...
#ifdef LOCATOR
		/*
		 * Was it the locator?  Normal exit can be ignored; we'll start a
		 * new one at the next iteration of the postmaster's main loop, if
		 * necessary.  Any other exit condition is treated as a crash.
		 */
		if (pid == LocatorPartitionLauncherPID)
		{
			LocatorPartitionLauncherPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("Locator partition launcher process"));
			continue;
		}
#endif
...
HandleChildCrash(int pid, int exitstatus, const char *procname)
{
	...
	if (pid == LocatorBgWriterPID)
		LocatorBgWriterPID = 0;
	else if (LocatorBgWriterPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) LocatorBgWriterPID)));
		signal_child(LocatorBgWriterPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
#endif
...
#ifdef LOCATOR
	/* Take care of the locator too */
	if (pid == LocatorPartitionLauncherPID)
		LocatorPartitionLauncherPID = 0;
	else if (LocatorPartitionLauncherPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) LocatorPartitionLauncherPID)));
		signal_child(LocatorPartitionLauncherPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
#endif
...
PostmasterStateMachine(void)
{
	...
	if (pmState == PM_STOP_BACKENDS)
	{
		...
		/* and the locator too */
		if (LocatorPartitionLauncherPID != 0)
			signal_child(LocatorPartitionLauncherPID, SIGTERM);
		/* and the locator bgwriter too */
		if (LocatorBgWriterPID != 0)
			signal_child(LocatorBgWriterPID, SIGTERM);
#endif
...
#endif
#ifdef LOCATOR
			Assert(LocatorPartitionLauncherPID == 0);
			Assert(LocatorBgWriterPID == 0);
#endif
			Assert(AutoVacPID == 0);
...
#ifdef LOCATOR
	if (LocatorPartitionLauncherPID != 0)
		signal_child(LocatorPartitionLauncherPID, signal);
	if (LocatorBgWriterPID != 0)
		signal_child(LocatorBgWriterPID, signal);
#endif
	if (WalReceiverPID != 0)
...
#ifdef LOCATOR
	if (CheckPostmasterSignal(PMSIGNAL_LOCATOR_PARTITIONING_WORKER) &&
		Shutdown <= SmartShutdown && pmState < PM_STOP_BACKENDS)
	{
		/*
		 * The locator partitioning launcher wants us to start a worker process
		 */
		StartLocatorPartitioningWorker();
	}
#endif /* LOCATOR */
...
#ifdef LOCATOR
/*
 * Fork LOCATOR partitioning worker.
 */
static void
StartLocatorPartitioningWorker(void)
{
	Backend    *bn;

	if (canAcceptConnections(BACKEND_TYPE_LOCATOR) == CAC_OK)
	{
		/*
		 * Compute the cancel key that will be assigned to this session. We
		 * probably don't need cancel keys for autovac workers, but we'd
		...
		if (bn)
		{
			...
			bn->pid = StartLocatorPartitionWorker();
			if (bn->pid > 0)
			{
				bn->bkend_type = BACKEND_TYPE_LOCATOR;
				dlist_push_head(&BackendList, &bn->elem);
#ifdef EXEC_BACKEND
				ShmemBackendArrayAdd(bn);
#endif
