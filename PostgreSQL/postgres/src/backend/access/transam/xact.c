...
#ifdef LSM_TXN
	if (fdw_relation_id_cnt > 0)
	{
		for (int i = 0; i < fdw_relation_id_cnt; i++)
		{
			fdw_commit(MyBackendId, fdw_relation_ids[i]);
			fdw_relation_ids[i] = InvalidOid;
		}
...
AbortTransaction(void)
{
	...
	if (fdw_relation_id_cnt > 0)
	{
		for (int i = 0; i < fdw_relation_id_cnt; i++)
		{
			fdw_abort(MyBackendId, fdw_relation_ids[i]);
			fdw_relation_ids[i] = InvalidOid;
		}
