...
#ifdef LOCATOR
	Assert(!(rel->rd_locator_partitioning) || (route_synopsis != NULL));
	slot->tts_locator_route_synopsis = route_synopsis;

	locator_init_columnar_slot_for_tuple_check(slot, rel);
#endif /* LOCATOR */
...
table_block_relation_size(Relation rel, ForkNumber forkNumber)
{
	uint64		nblocks = 0;
 	...
 	if (forkNumber == InvalidForkNumber)
 	{
 		...
 		if (rel->rd_locator_partitioning)
 		{
 			i = 1;
 			nblocks = LocatorGetNBlocks(rel);
 		}
 		else
 			i = 0;
 	...
 	{
 		if (rel->rd_locator_partitioning && forkNumber == MAIN_FORKNUM)
 			nblocks = LocatorGetNBlocks(rel);
 		else
