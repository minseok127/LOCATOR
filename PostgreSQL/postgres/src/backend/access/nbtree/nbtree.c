		 ...
		 IndexInfo *indexInfo)
{
	...
	if (heapRel->rd_locator_partitioning)
	{
		/* Load the locator info and lvl_cnt of new record */
		locator_route_synopsis = (LocatorRouteSynopsis)(values[INDEX_MAX_KEYS]);
		lvl_cnt = (int32)(values[INDEX_MAX_KEYS + 1]);
	}
	else
	{
		locator_route_synopsis = NULL;
		lvl_cnt = -1;
	}
#endif /* LOCATOR */
	...
	if (locator_route_synopsis != NULL)
	{
		...
		Assert(heapRel->rd_locator_partitioning);

		hoff = IndexInfoFindDataOffset(itup->t_info);
		data_size = heap_compute_data_size(RelationGetDescr(rel),
									   values, isnull);

		tup_size = hoff + data_size;
		size = SizeOfLocatorRouteSynopsisData(lvl_cnt);
		
		if (tup_size + size > INDEX_SIZE_MASK)
			ereport(ERROR, errmsg("Locator: index tuple is too big"));

		IndexTupleSetRouteSynopsisOffset(itup, tup_size);
		// TODO: fix it
		itup->t_tid.ip_posid = 1;
	...
	result = _bt_doinsert(rel, itup, checkUnique, indexUnchanged, heapRel,
						  lvl_cnt, locator_route_synopsis);
#else
	itup->t_tid = *ht_ctid;
