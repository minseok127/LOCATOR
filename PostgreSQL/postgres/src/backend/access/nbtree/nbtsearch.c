...
#ifdef LOCATOR
						, bool isLocator, int lastPartitionLevel, int mempart_rec_cnt
#endif
						);
...
readcomplete:
	...
	if (scan->xs_heapfetch && IsLocator(scan->xs_heapfetch->rel))
		scan->xs_route_synopsis = &(currItem->routeSynopsis);
#endif
...
_bt_readpage(IndexScanDesc scan, ScanDirection dir, OffsetNumber offnum)
{
	...
	{
		isLocator = IsLocator(scan->xs_heapfetch->rel);

		if (isLocator)
			mempart_rec_cnt = (scan->xs_heapfetch->rel->records_per_block * LocatorMempartPageCount);
	}
#endif
			...
			if (_bt_checkkeys(scan, itup, indnatts, dir, &continuescan))
			{
				...
				if (!BTreeTupleIsPosting(itup))
				{
					...
					_bt_saveitem(so, itemIndex, offnum, itup, 
										isLocator, scan->lastPartitionLevel,
										mempart_rec_cnt);
#else
					_bt_saveitem(so, itemIndex, offnum, itup);
#endif
			...
			)
{
	...
	if (isLocator)
	{
		memcpy(((char *) currItem) + offsetof(BTScanPosItem, routeSynopsis), 
				GetLocatorRouteSynopsis(itup),
				SizeOfLocatorRouteSynopsisData(lastPartitionLevel + 1));
	}
#endif 
