...
static TransactionId _bt_check_unique(Relation rel, BTInsertState insertstate,
									  ...
									  LocatorRouteSynopsis route_synopsis,
...
search:
	...
	if (checkingunique)
	{
		...
		xwait = _bt_check_unique(rel, &insertstate, heapRel, checkUnique,
								 &is_unique, locator_route_synopsis,
								 &speculativeToken);
#else /* LOCATOR */
		xwait = _bt_check_unique(rel, &insertstate, heapRel, checkUnique,
	...
	if (checkUnique != UNIQUE_CHECK_EXISTING)
	{
		...
		_bt_insertonpg(rel, itup_key, insertstate.buf, InvalidBuffer, stack,
					   ...
					   insertstate.postingoff, false, locator_route_synopsis);
#else
		_bt_insertonpg(rel, itup_key, insertstate.buf, InvalidBuffer, stack,
					   itup, insertstate.itemsz, newitemoff,
...
_bt_check_unique(Relation rel, BTInsertState insertstate, Relation heapRel,
				 IndexUniqueCheck checkUnique, bool *is_unique,
				 LocatorRouteSynopsis newRouteSynopsis,
				 uint32 *speculativeToken)
#else /* !LOCATOR */
static TransactionId
_bt_check_unique(Relation rel, BTInsertState insertstate, Relation heapRel,
...
#ifdef LOCATOR
					route_synopsis = GetLocatorRouteSynopsis(curitup);
#endif /* LOCATOR */
				}
...
#endif /* LOCATOR */
{
	...
	{
#ifdef LOCATOR
		bool isLocator = (locator_route_synopsis != NULL);
#endif
		Buffer		rbuf;
		...
		rbuf = _bt_split(rel, itup_key, buf, cbuf, newitemoff, itemsz, itup,
						origitup, nposting, isLocator, locator_route_synopsis, 
																postingoff);
#else
		/* split the buffer into left and right halves */
		rbuf = _bt_split(rel, itup_key, buf, cbuf, newitemoff, itemsz, itup,
	...
	else
	{
...
#ifdef LOCATOR
		if (locator_route_synopsis != NULL && isleaf)
		{
			if (PageAddItemWithRouteSynopsis(page, (Item) itup, itemsz, newitemoff,
								false, false, IndexTupleGetRouteSynopsisOffset(itup),
								locator_route_synopsis) == InvalidOffsetNumber)
		...
		else
		{
			if (locator_route_synopsis != NULL)
				itemsz = MAXALIGN(IndexTupleGetRouteSynopsisOffset(itup));
		...
		else if (i == newitemoff)
		{
			if (newitemonleft)
			{
				...
				if (!_bt_pgaddtup(leftpage, newitemsz, newitem, afterleftoff,
								  isLocator, locator_route_synopsis,
								  false))
#else
				if (!_bt_pgaddtup(leftpage, newitemsz, newitem, afterleftoff,
								  										false))
			 ...
			 bool newfirstdataitem)
{
	...
	if (isLocator && (locator_route_synopsis == NULL))
		locator_route_synopsis = GetLocatorRouteSynopsis(itup);
#endif
	...
	if (isLocator && isleaf)
	{
		if (unlikely(PageAddItemWithRouteSynopsis(page, (Item) itup, itemsize, itup_off,
								false, false, IndexTupleGetRouteSynopsisOffset(itup),
								locator_route_synopsis) == InvalidOffsetNumber))
			return false;
	}
	...
	{
		if (isLocator && !newfirstdataitem)
			itemsize = MAXALIGN(IndexTupleGetRouteSynopsisOffset(itup));
