...
#endif
#ifdef LOCATOR
	size = add_size(size, LocatorPartitioningShmemSize());
	size = add_size(size, LocatorExternalCatalogShmemSize());
	size = add_size(size, NLocatorMempartCount * sizeof(LWLock));
