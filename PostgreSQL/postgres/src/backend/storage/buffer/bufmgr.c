...
/* TODO: LocatorBgWriter flushes dirty mempartitions, then is this function necessary? */
#if 0
static void
LocatorMempartSync(int flags)
{
	...
	/* TODO: only at shutdown? or every checkpoint? */
	if (!is_shutdown)
		return;
...
ckpt_buforder_comparator(const CkptSortItem *a, const CkptSortItem *b)
{
	...
	/* TODO: Does generation number have to be compared? */
	if (a->partGen < b->partGen)
		return -1;
	else if (a->partGen > b->partGen)
		return 1;
#endif /* LOCATOR */
	...
	while (PrefetchDescGetNeedPrefetch(prefetchDesc))
	{
		...
		for (idx = 0; idx < MAX_PAGE_IN_CHUNK; idx++)
		{
			...
			if (unlikely(reqPageId > lastPageId))
			{
				...
				if ((pdesc->c_fd = LocatorSimpleOpen(path, O_RDWR)) == -1)
				{
...
/*
 * LocatorMempartitionFlush -- Write out all dirty mempartitions in the pool.
 */
void
