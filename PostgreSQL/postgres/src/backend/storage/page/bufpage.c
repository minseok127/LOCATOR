...
void
LocatorPartitionPageInit(Page page, Size pageSize, Size tupleSize,
						 uint16 tuplesPerRecordZone)
{
	...
	/*
	 * pd_lower and pd_upper of locator partition page
	 * 
	 * SIRO tuple: (column group 0 of ROW_ORIENTED and MIXED layout)
	 * 		pd_lower: same as vanilla PostgreSQL
	 * 		pd_upper: same as vanilla PostgreSQL
	 * 		pd_special: same as vanilla PostgreSQL
	 * 	
	 * columnar tuple: (else)
	 * 		pd_lower: start point of null bit array
	 * 		pd_upper: start point of data
	 * 		pd_special: count of tuples
	 * 
	 * For SIRO tuples, the tupleIndex coming in as a parameter is 0.
	 */

	p->pd_flags = tupleSize == 0 ? 0 : PD_COLUMNAR;
	p->pd_lower = SizeOfPageHeaderData;
	p->pd_upper = pageSize - tupleSize * tuplesPerRecordZone;
	p->pd_special = tupleSize == 0 ? pageSize : 0;
...
#ifdef LOCATOR
	/*
	 * In LOCATOR, heap_insert() may not block SeqScan.
							 ...
							 int flags)
{
	...
	/*
	 * In LOCATOR, locator_insert() may not block SeqScan.
	 * Because itemId is updated atomically in LOCATOR when a new tuple is
	...
	*((uint32*)p_pointer) = phdr->pd_gen;
	...
	if (locator_route_synopsis != NULL)
		*((LocatorRecordKey *)p_pointer) = locator_route_synopsis->rec_key;
...
#ifdef LOCATOR
OffsetNumber
PageAddItemWithRouteSynopsisExtended(Page page,
								   Item item,
								   Size size,
								   OffsetNumber offsetNumber,
								   int flags,
								   ...
								   LocatorRouteSynopsis locator_route_synopsis)
{
	...
	memcpy((char *) page + upper + tup_size, locator_route_synopsis,
												(size - tup_size));
