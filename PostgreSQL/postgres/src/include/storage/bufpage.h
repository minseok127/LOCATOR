...
#define PD_ALL_VISIBLE		0x0004	/* all tuples on page are visible to
									 * everyone */
#ifdef LOCATOR
#define PD_COLUMNAR			0x0008	/* columnar page */
...
#ifdef LOCATOR
#define PageIsCOLUMNAR(page) \
	(((PageHeader) (page))->pd_flags & PD_COLUMNAR)
#endif /* LOCATOR */
...
/**
 * ----------------------------------------------------------------------------------
 *   8-byte   |    8-byte   |   4-byte  | 4-byte |     5-byte      |     3-byte     |
 * left-pleaf | right-pleaf | xid-bound | pd_gen | locator_seqnum  | locator_partkey|
 * ----------------------------------------------------------------------------------
 * |------------------- header ------------------|----------- record key -----------|
 */

#define PITEM_SZ (32)
#define PITEM_LOCATOR_RECORDKEY_OFF (24)

#define PGetLocatorRecordKey(plocator) \
	((LocatorRecordKey *) (((char *) plocator) + PITEM_LOCATOR_RECORDKEY_OFF))
 ...
 * left-pleaf | right-pleaf | xid-bound | padded |
 * -----------------------------------------------
 * |------------------- header ------------------|
 */
...
#define PageAddItemPartition(page, item, size, offsetNumber, \
							 ...
							 locator_route_synopsis, isInit) \
		PageAddItemPartitionExtended(page, item, size, offsetNumber, \
									 blockNum, maxlen, locator_route_synopsis, isInit, \
									 ((overwrite) ? PAI_OVERWRITE : 0) | \
									 ((is_heap) ? PAI_IS_HEAP : 0))
#else /* !LOCATOR*/
#define PageAddItemWithDummy(page, item, size, offsetNumber, \
...
#ifdef LOCATOR
#define PageAddItemWithRouteSynopsis(page, item, size, offsetNumber, overwrite, \
								   is_heap, tup_size, locator_route_synopsis) \
	PageAddItemWithRouteSynopsisExtended(page, item, size, offsetNumber, \
								   ((overwrite) ? PAI_OVERWRITE : 0) | \
								   ((is_heap) ? PAI_IS_HEAP : 0), tup_size, locator_route_synopsis)
#endif /* LOCATOR */
...
#ifdef LOCATOR
extern void LocatorPartitionPageInit(Page page, Size pageSize, Size tupleSize,
...
#ifdef LOCATOR
extern OffsetNumber PageAddItemWithRouteSynopsisExtended(Page page, Item item,
													   Size size,
													   OffsetNumber offsetNumber,
													   int flags,
													   Size tup_size,
