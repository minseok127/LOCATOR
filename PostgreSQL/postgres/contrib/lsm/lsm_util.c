...
#define CALL_PREVIOUS_UTILITY(parseTree, queryString, context, readOnlyTree, \
							  paramListInfo, destReceiver, completionTag) \
		PREVIOUS_UTILITY(plannedStmt, queryString, context, readOnlyTree, \
...
#ifdef LASER
	char *current;
	if ((char)(datum) == '-') // unchanged value
	{
		enlargeStringInfo(buffer, buffer->len + 1); // 1 is for delimiter
		current = buffer->data + buffer->len;
	}
	else // real null
	{
		enlargeStringInfo(buffer, buffer->len + HEADERBUFFSIZE + 1); // 1 is for delimiter
		current = buffer->data + buffer->len;
							   ...
							   const char *queryString)
{
	...
	CopyFromState copyState = BeginCopyFrom(pstate,
											relation,
											NULL,
											copyStmt->filename,
											copyStmt->is_program,
